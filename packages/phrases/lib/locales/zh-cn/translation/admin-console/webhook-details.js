const webhook_details = {
    ...
    settings: {
        settings: '设置',
        settings_description: 'Webhook 允许您通过向端点 URL 发送 POST 请求，实时接收特定事件的更新。这使您可以根据接收到的新信息立即采取行动。',
        events: '事件',
        ...
        signing_key: '签名密钥',
        signing_key_tip: '将由 Logto 提供的密钥作为请求标头添加到您的端点中，以确保 Webhook 负载的真实性。',
        regenerate: '重新生成',
        regenerate_key_title: '重新生成签名密钥',
        regenerate_key_reminder: '是否确定要修改签名密钥？重新生成后将立即生效。请在您的端点中同步修改签名密钥。',
        regenerated: '签名密钥已重新生成。',
        custom_headers: '自定义标头',
        custom_headers_tip: '选择性地，您可以向 Webhook 负载添加自定义标头，以提供事件的其他上下文或元数据。',
        key_duplicated_error: 'Key 不能重复。',
        key_missing_error: '必须填写 Key。',
        value_missing_error: '必须填写值。',
        test: '测试',
        test_webhook: '测试您的 Webhook',
        test_webhook_description: '配置 Webhook，并使用每个选定事件的负载示例进行测试，以验证正确的接收和处理。',
