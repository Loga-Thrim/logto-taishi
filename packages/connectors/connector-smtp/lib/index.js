...
const smtpConfigGuard = z.object({
    host: z.string(),
    port: z.number(),
    auth: authGuard,
    fromEmail: z.string(),
...
const sendMessage = (getConfig) => async (data, inputConfig) => {
    ...
    const contentsObject = parseContents(typeof payload.code === 'string'
        ? template.content.replaceAll(/{{\s*code\s*}}/g, payload.code)
        : template.content, template.contentType);
    const mailOptions = {
        to,
    ...
    try {
        ...
        throw new ConnectorError(ConnectorErrorCodes.General, error instanceof Error ? error.message : '');
    }
};
const parseContents = (contents, contentType) => {
    switch (contentType) {
        ...
        default: {
            throw new ConnectorError(ConnectorErrorCodes.InvalidConfig, '`contentType` should be ContextType.');
        }
    }
};
const createSmtpConnector = async ({ getConfig }) => {
    return {
...
export { createSmtpConnector as default };
