...
const remove = {
    ...
    builder: (yargs) => yargs.positional('packages', {
    ...
    handler: async ({ path: inputPath, packages: packageNames }) => {
        ...
        const notFoundPackageNames = packageNames.filter((current) => !existingPackages.some(({ name }) => current === name));
        if (notFoundPackageNames.length > 0) {
            consoleLog.fatal(`Cannot remove ${notFoundPackageNames
                .map((name) => chalk.green(name))
                .join(', ')}: not found in your Logto instance directory`);
        }
        const okSymbol = Symbol('Connector removed');
        const result = await Promise.all(packageNames.map(async (current) => {
