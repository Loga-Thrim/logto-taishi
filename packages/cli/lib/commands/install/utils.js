...
import { cliConfig, ConfigKey, consoleLog, downloadFile, isTty, oraPromise, safeExecSync, } from '../../utils.js';
...
export const validateNodeVersion = () => {
    ...
    if (required.every((version) => version.major !== current.major)) {
        consoleLog.fatal(`Logto requires NodeJS ${requiredVersionString}, but ${current.version} found.`);
    }
    if (required.some((version) => version.major === current.major && version.compare(current) > 0)) {
        consoleLog.warn(`Logto is tested under NodeJS ${requiredVersionString}, but version ${current.version} found.`);
    }
};
const validatePath = (value) => existsSync(path.resolve(value))
    ? `The path ${chalk.green(value)} already exists, please try another.`
    : true;
export const inquireInstallPath = async (initialPath) => {
    if (!isTty()) {
        assert(initialPath, new Error('Path is missing'));
        return initialPath;
    }
    const { instancePath } = await inquirer.prompt({
        name: 'instancePath',
...
export const validateDatabase = async () => {
    ...
    const { hasPostgresUrl } = await inquirer.prompt({
        name: 'hasPostgresUrl',
...
const fetchDownloadUrl = async (url) => {
    ...
    try {
        ...
        if (error instanceof RequestError && error.response?.statusCode === 404) {
            consoleLog.warn(`Current version "v${packageJson.version}" not found in GitHub Releases, fallback to "latest".\n` +
...
export const downloadRelease = async (url) => {
...
export const decompress = async (toPath, tarPath) => {
    const run = async () => {
        try {
    ...
    return oraPromise(run(), {
        text: `Decompress to ${toPath}`,
    }, true);
};
export const seedDatabase = async (instancePath, cloud) => {
    try {
        ...
        consoleLog.fatal('Error occurred during seeding your Logto database. Nothing has changed since the seeding process was in a transaction.\n\n' +
            `  To skip the database seeding, append ${chalk.green('--skip-seed')} to the command options.`);
    }
};
export const createEnv = async (installPath, databaseUrl) => {
...
export const logFinale = (installPath) => {
    const startCommand = `cd ${installPath} && npm start`;
    consoleLog.info(`Use the command below to start Logto. Happy hacking!\n\n  ${chalk.green(startCommand)}`);
};
