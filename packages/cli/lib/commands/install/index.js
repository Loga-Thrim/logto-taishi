...
import { validateNodeVersion, inquireInstallPath, validateDatabase, downloadRelease, seedDatabase, createEnv, logFinale, decompress, isUrl, } from './utils.js';
const installLogto = async ({ path, skipSeed, downloadUrl, cloud }) => {
    validateNodeVersion();
    ...
    const tarPath = !downloadUrl || isUrl(downloadUrl) ? await downloadRelease(downloadUrl) : downloadUrl;
    await decompress(installPath, tarPath);
    // Seed database
    if (skipSeed) {
        consoleLog.info(`Skipped database seeding.\n\n' + '  You can use the ${chalk.green('db seed')} command to seed database when ready.\n`);
    }
    else {
