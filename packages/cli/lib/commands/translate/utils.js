...
export const readBaseLocaleFiles = async (directory) => {
...
export const createFullTranslation = async ({ instancePath, packageName, languageTag, verbose = true, queue = new PQueue({ concurrency: 5 }), }) => {
    const localeFiles = await getBaseAndTargetLocaleFiles(instancePath, packageName, languageTag);
    if (verbose) {
        consoleLog.info('Found ' +
...
export const syncTranslation = async ({ instancePath, packageName, languageTag, verbose = true, queue = new PQueue({ concurrency: 5 }), }) => {
    const localeFiles = await getBaseAndTargetLocaleFiles(instancePath, packageName, languageTag);
    if (verbose) {
        consoleLog.info('Found ' +
            String(localeFiles.length) +
    ...
    for (const { baseLocaleFile, targetLocaleFile } of localeFiles) {
        ...
        if (!currentContent.includes(untranslatedMark)) {
            if (verbose) {
                consoleLog.info(`Target path ${targetLocaleFile} exists and has no untranslated mark, skipping`);
            }
            continue;
        }
...
const getBaseAndTargetLocaleFiles = async (instancePath, packageName, languageTag) => {
    ...
    return baseLocaleFiles.map((baseLocaleFile) => {
        const basePath = path.relative(path.join(directory, baseLanguage.toLowerCase()), baseLocaleFile);
        return {
            baseLocaleFile,
...
const createLocaleFile = async ({ api, baseLocaleFile, targetPath, targetLanguage, queue, }) => queue.add(async () => {
