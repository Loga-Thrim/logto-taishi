...
const create = {
    ...
    describe: 'Create a new language translation using ChatGPT. Note the environment variable `OPENAI_API_KEY` is required to work.',
    builder: (yargs) => yargs.positional('language-tag', {
    ...
    handler: async ({ path: inputPath, languageTag }) => {
        if (!isLanguageTag(languageTag)) {
            consoleLog.fatal('Invalid language tag. Run `logto translate list-tags` to see available list.');
        }
        ...
        if (isPhrasesUiBuiltInLanguageTag(languageTag)) {
            consoleLog.info(languageTag + ' is a built-in tag of phrases-experience, updating untranslated phrases');
        }
        await createFullTranslation({
            instancePath,
