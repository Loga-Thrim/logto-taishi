...
import { createDefaultAdminConsoleConfig, defaultTenantId, adminTenantId, defaultManagementApi, createAdminDataInAdminTenant, createMeApiInAdminTenant, createDefaultSignInExperience, createAdminTenantSignInExperience, createDefaultAdminConsoleApplication, createCloudApi, createTenantApplicationRole, CloudScope, } from '@logto/schemas';
...
export const createTables = async (connection) => {
    ...
    const queries = await Promise.all(tableFiles.map(async (file) => [
        file,
        await readFile(path.join(tableDirectory, file), 'utf8'),
    ]));
    const runLifecycleQuery = async (lifecycle, parameters = {}) => {
        const query = queries.find(([file]) => file.slice(1, -4) === lifecycle)?.[1];
        if (query) {
            await connection.query(sql `${raw(
...
export const seedTables = async (connection, latestTimestamp, isCloud) => {
    ...
    await assignScopesToRole(connection, adminTenantId, applicationRole.id, ...cloudAdditionalScopes
        .filter(({ name }) => name === CloudScope.SendSms || name === CloudScope.SendEmail)
        .map(({ id }) => id));
    // Assign all cloud API scopes to role `admin:admin`
    await assignScopesToRole(connection, adminTenantId, adminAdminData.role.id, ...cloudAdditionalScopes.map(({ id }) => id));
    ...
    await Promise.all([
        ...
        connection.query(insertInto(createDefaultSignInExperience(defaultTenantId, isCloud), 'sign_in_experiences')),
...
export const seedCloud = async (connection) => {
    await Promise.all([
