import { adminConsoleApplicationId, adminTenantId, defaultTenantId, createTenantMachineToMachineApplication, createAdminTenantApplicationRole, createCloudConnectionConfig, AdminTenantRole, } from '@logto/schemas';
 ...
 */
export const appendAdminConsoleRedirectUris = async (pool) => {
    const redirectUris = new GlobalValues().cloudUrlSet
        .deduplicated()
        .flatMap((endpoint) => [defaultTenantId, adminTenantId].map((tenantId) => appendPath(endpoint, tenantId, 'callback')));
 ...
 * - admin
 */
export const seedTenantCloudServiceApplication = async (pool, tenantId) => {
    ...
    const { id: tenantApplicationRoleId } = await pool.one(sql `
    select id from roles
    where name = ${AdminTenantRole.TenantApplication}
    and tenant_id = ${adminTenantId}
  `);
    await pool.query(insertInto(createAdminTenantApplicationRole(cloudServiceApplication.id, tenantApplicationRoleId), 'applications_roles'));
    // Create Cloud Service M2M application logto_config
    await pool.query(insertInto(createCloudConnectionConfig(tenantId, cloudServiceApplication.id, cloudServiceApplication.secret), 'logto_configs'));
