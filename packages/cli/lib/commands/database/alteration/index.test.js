...
const { getCurrentDatabaseAlterationTimestamp } = await mockEsmWithActual('../../../queries/system.js', () => ({
...
describe('chooseAlterationsByVersion()', () => {
    ...
    it('chooses nothing when input version is invalid', async () => {
        await expect(chooseAlterationsByVersion(files, 'next1')).rejects.toThrow(new TypeError('Invalid Version: next1'));
        await expect(chooseAlterationsByVersion([], 'ok')).rejects.toThrow(new TypeError('Invalid Version: ok'));
    });
    it('chooses correct alteration files', async () => {
        await Promise.all([
            ...
            expect(chooseAlterationsByVersion(files, 'v1.0.0-beta.10')).resolves.toEqual(files.slice(0, 3)),
