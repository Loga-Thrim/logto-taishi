...
export const getDatabaseUrlFromConfig = async () => (await getCliConfigWithPrompt({
...
export const insertInto = (object, table) => {
    const keys = Object.keys(object);
    return sql `
    insert into ${sql.identifier([table])}
    (${sql.join(keys.map((key) => sql.identifier([decamelize(key)])), sql `, `)})
    values (${sql.join(keys.map((key) => convertToPrimitiveOrSql(key, object[key] ?? null)), sql `, `)})
  `;
};
