...
export const safeExecSync = (command) => {
    try {
...
export const getPathInModule = (moduleName, relativePath = '/') => 
// https://stackoverflow.com/a/49455609/12514940
path.join(path.dirname(createRequire(import.meta.url).resolve(`${moduleName}/package.json`)), relativePath);
export const oraPromise = async (promise, options, exitOnError = false) => {
...
(function (ConfigKey) {
    ConfigKey["DatabaseUrl"] = "DB_URL";
})(ConfigKey || (ConfigKey = {}));
export const cliConfig = new Map();
export const getCliConfigWithPrompt = async ({ key, readableKey, comments, defaultValue, }) => {
...
const buildPathErrorMessage = (value) => `The path ${chalk.green(value)} does not contain a Logto instance. Please try another.`;
...
export const inquireInstancePath = async (initialPath) => {
    const inquire = async () => {
...
const getConnectorPackageName = async (directory) => {
...
export const getConnectorPackagesFromDirectory = async (directory) => {
    const content = await readdir(directory, 'utf8');
    const rawPackages = await Promise.all(content.map(async (value) => {
    ...
    return rawPackages.filter((packageInfo) => typeof packageInfo.name === 'string');
};
...
packageName) => {
    ...
    await Promise.all(targetPackages.map(async (packageName) => {
        const phrasesPath = path.join(instancePath, 'packages', packageName);
        const localesPath = path.join(phrasesPath, 'src/locales');
        await execPromise('pnpm', ['eslint', '--ext', '.ts', path.relative(phrasesPath, localesPath), '--fix'], { cwd: phrasesPath });
    }));
