...
describe('sign-in experience: sign-in preview', () => {
    ...
    it('navigate to sign-in experience page', async () => {
        ...
        await expect(page).toMatchElement('div[class$=main] div[class$=container] div[class$=cardTitle] div[class$=titleEllipsis]', {
    ...
    it('switch between theme modes', async () => {
        // Enable dark mode
        await expect(page).toClick('form div[class$=field] label[class$=switch]:has(input[name="color.isDarkModeEnabled"])');
        // Wait for the preview to update since we don't have a reliable way to match the dark mode icon
        await page.waitForTimeout(500);
        // Switch to dark mode
        await expect(page).toClick('div[class$=preview] div[class$=header] div[class$=selects] button:first-of-type');
        await expect(page).toMatchElement('div[class$=preview] div[class$=deviceWrapper] div[class*=device][class*=dark]');
        // Switch to light mode
        await expect(page).toClick('div[class$=preview] div[class$=header] div[class$=selects] button:first-of-type');
        await expect(page).toMatchElement('div[class$=preview] div[class$=deviceWrapper] div[class*=device][class*=light]');
        // Reset
        await expect(page).toClick('form div[class$=field] label[class$=switch]:has(input[name="color.isDarkModeEnabled"])');
    });
    ...
    it('check the live preview', async () => {
        const livePreviewPagePromise = new Promise((resolve) => {
...
//# sourceMappingURL=sign-in-preview.test.js.map