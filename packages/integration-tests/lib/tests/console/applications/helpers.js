...
import { expectModalWithTitle, expectToClickDetailsPageOption, expectToClickModalAction, expectToOpenNewPage, waitForToast, } from '#src/ui-helpers/index.js';
import { expectNavigation } from '#src/utils.js';
import { frameworkGroupLabels } from './constants.js';
export const expectFrameworksInGroup = async (page, groupSelector) => {
...
export const expectToChooseAndClickApplicationFramework = async (page, framework) => {
    const frameworkCard = await expect(page).toMatchElement('div[class*=card]:has(div[class$=header] div[class$=name])', {
        text: framework,
    });
    await expect(frameworkCard).toClick('button span', { text: 'Start Building' });
};
export const expectFrameworkExists = async (page, framework) => {
    await expect(page).toMatchElement('div[class*=card]:has(div[class$=header] div[class$=name])', {
        text: framework,
    });
};
export const expectToProceedApplicationCreationFrom = async (page, { name, description }) => {
...
export const expectToProceedSdkGuide = async (page, { guideFilename, sample, redirectUri, postSignOutRedirectUri }, skipFillForm = false) => {
    ...
    await expectToOpenNewPage(browser, appendPath(new URL('https://github.com/logto-io'), sample.repo, 'tree/HEAD', sample.path).href);
    if (!skipFillForm) {
        const redirectUriFieldWrapper = await expect(page).toMatchElement('div[class$=wrapper]:has(>div[class$=field]>div[class$=headline]>div[class$=title])', { text: 'Redirect URI' });
        ...
        const postSignOutRedirectUriWrapper = await expect(page).toMatchElement('div[class$=wrapper]:has(>div[class$=field]>div[class$=headline]>div[class$=title])', { text: 'Post Sign-out Redirect URI' });
...
export const expectToProceedAppDeletion = async (page, appName) => {
