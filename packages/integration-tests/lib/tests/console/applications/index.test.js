...
import { expectConfirmModalAndAct, expectModalWithTitle, expectToClickModalAction, expectToDiscardChanges, expectToOpenNewPage, expectToSaveChanges, expectUnsavedChangesAlert, goToAdminConsole, waitForToast, } from '#src/ui-helpers/index.js';
import { expectNavigation, appendPathname } from '#src/utils.js';
import { applicationTypesMetadata, initialApp, testApp, } from './constants.js';
import { expectFrameworkExists, expectToChooseAndClickApplicationFramework, expectToProceedApplicationCreationFrom, expectToProceedSdkGuide, expectToProceedAppDeletion, expectFrameworksInGroup, } from './helpers.js';
...
describe('applications', () => {
    ...
    it('navigate to applications page', async () => {
        await expectNavigation(page.goto(appendPathname('/console/applications', logtoConsoleUrl).href));
    ...
    it('the table placeholder should be rendered correctly', async () => {
        await expect(page).toMatchElement('div[class$=guideLibraryContainer] div[class$=titleEllipsis]', { text: 'Select a framework or tutorial', timeout: 2000 });
    ...
    it('create the initial application from the table placeholder', async () => {
        ...
        await expectNavigation(expect(page).toClick('div[class$=main] a[class$=backLink]', {
    ...
    it('can open the logto github repo issue page when click on the framework not found button', async () => {
        ...
        await expect(page).toClick('.ReactModalPortal div[class$=header] button[class$=requestSdkButton]');
        await expectToOpenNewPage(browser, 'https://github.com/logto-io/logto/issues');
        // Return to the application list page
        await expectNavigation(expect(page).toClick('.ReactModalPortal div[class$=header] button:has(svg[class$=closeIcon])'));
        expect(page.url()).toBe(new URL('/console/applications', logtoConsoleUrl).href);
    });
    it('can create an application by framework from the app creation modal and modify its data', async () => {
        ...
        // Close guide
        await expect(page).toClick('.ReactModalPortal div[class$=drawerContainer] div[class$=header] button:last-of-type');
        ...
        const redirectUriFiled = await expect(page).toMatchElement('div[class$=field]:has(>div[class$=headline]>div[class$=title]', { text: 'Redirect URIs' });
        ...
        await page.waitForSelector('div:has(>div[class$=deletableInput]):last-of-type button:has(svg[class$=minusIcon])');
        await expect(redirectUriFiled).toFill('div:has(>div[class$=deletableInput]):last-of-type input', `${testApp.redirectUri}/v2`);
        await expectToSaveChanges(page);
        await waitForToast(page, { text: 'Saved' });
        // Click delete button
        await expect(redirectUriFiled).toClick('div:has(>div[class$=deletableInput]):last-of-type button:has(svg[class$=minusIcon])');
        ...
        await expectToSaveChanges(page);
        // Expect error
        await expect(page).toMatchElement('div[class$=field] div[class$=multilineInput] div[class$=error]', {
    ...
    it.each(applicationTypesMetadata)('can create and modify a(n) $type application without framework', async (app) => {
        await expect(page).toClick('div[class$=main] div[class$=headline] button span', {
            text: 'Create Application',
        });
