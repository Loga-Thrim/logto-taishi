...
import { goToAdminConsole, expectToSaveChanges, waitForToast, expectToDiscardChanges, expectModalWithTitle, expectToClickModalAction, } from '#src/ui-helpers/index.js';
import { appendPathname, expectNavigation, formatPhoneNumberToInternational, generateEmail, generateName, generatePhone, generateUsername, } from '#src/utils.js';
...
describe('user management', () => {
    ...
    it('can create a new user', async () => {
        ...
        const userId = await page.$eval('div[class$=main] div[class$=metadata] div[class$=row] div:first-of-type', (element) => element.textContent);
        if (userId) {
            expect(page.url()).toBe(new URL(`console/users/${userId}/settings`, logtoConsoleUrl).href);
        }
        const email = await page.$eval('form input[name=primaryEmail]', (element) => element instanceof HTMLInputElement ? element.value : null);
        const phone = await page.$eval('form input[name=primaryPhone]', (element) => element instanceof HTMLInputElement ? element.value : null);
        const username = await page.$eval('form input[name=username]', (element) => element instanceof HTMLInputElement ? element.value : null);
