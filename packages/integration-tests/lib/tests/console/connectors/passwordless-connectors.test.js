...
import { expectToClickDetailsPageOption, expectUnsavedChangesAlert, goToAdminConsole, expectToSaveChanges, waitForToast, } from '#src/ui-helpers/index.js';
import { expectNavigation, appendPathname } from '#src/utils.js';
import { expectToConfirmConnectorDeletion, expectToSelectConnector, findNextCompatibleConnector, waitForConnectorCreationGuide, } from './helpers.js';
...
describe('passwordless connectors', () => {
    ...
    it.each(passwordlessConnectorTestCases)('can setup and modify a(n) $factoryId connector', async (connector) => {
        const { factoryId, isEmailConnector, name, initialFormData, updateFormData, errorFormData } = connector;
        ...
        // Try send test
        await expect(page).toFill('input[name=sendTo]', isEmailConnector ? 'fake@email.com' : '+1 555-123-4567');
        ...
        if (nextConnector) {
            await expectToClickDetailsPageOption(page, isEmailConnector ? 'Change email connector' : 'Change SMS connector');
            await expectToSelectConnector(page, {
...
//# sourceMappingURL=passwordless-connectors.test.js.map