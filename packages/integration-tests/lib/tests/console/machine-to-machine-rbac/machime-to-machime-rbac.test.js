...
import { expectConfirmModalAndAct, expectModalWithTitle, expectToClickModalAction, goToAdminConsole, waitForToast, } from '#src/ui-helpers/index.js';
import { expectNavigation, appendPathname, generateResourceName, generateResourceIndicator, generateScopeName, generateRoleName, } from '#src/utils.js';
import { expectToChooseAndClickApplicationFramework, expectToProceedApplicationCreationFrom, } from '../applications/helpers.js';
...
describe('M2M RBAC', () => {
    ...
    describe('create api resource and permissions', () => {
        it('navigate to api resources page', async () => {
            await expectNavigation(page.goto(appendPathname('/console/api-resources', logtoConsoleUrl).href));
            await expect(page).toMatchElement('div[class$=main] div[class$=headline] div[class$=titleEllipsis]', {
                text: 'API Resources',
            });
        });
    ...
    describe('create m2m role', () => {
        ...
        it('delete a permission from a role on the role details page', async () => {
            ...
            const permissionRow = await expect(page).toMatchElement('table tbody tr:has(td div[class$=name])', { text: permissionName });
    ...
    describe('assign a role to a m2m app (on role details page)', () => {
        it('assign a role to a m2m app on the role details page', async () => {
            ...
            await expect(page).toClick('.ReactModalPortal div[class$=rolesTransfer] div[class$=item] div[class$=title]', {
            ...
            await expect(page).toMatchElement('div[class$=applicationsTable] td div[class$=item] a[class$=title]', {
                text: rbacTestAppname,
            });
        });
    });
    describe('assign/remove a role to/from a m2m app (on m2m app details page)', () => {
        ...
        it('add a role to m2m app on the application details page', async () => {
            ...
            await expect(page).toClick('.ReactModalPortal div[class$=rolesTransfer] div[class$=item] div[class$=name]', {
                text: roleName,
            });
...
//# sourceMappingURL=machime-to-machime-rbac.test.js.map