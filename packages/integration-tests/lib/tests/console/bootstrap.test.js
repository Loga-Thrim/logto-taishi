...
describe('smoke testing for console admin account creation and sign-in', () => {
    ...
    it('can sign out of admin console', async () => {
        ...
        await expectNavigation(expect(page).toClick('.ReactModalPortal div[class$=dropdownContainer] div[class$=dropdownItem]:last-child'));
    ...
    it('can highlight the current tab in the sidebar', async () => {
        ...
        await expectNavigation(expect(page).toClick([dcls('sidebar'), 'a' + cls('row')].join(' '), {
            text: 'Dashboard',
        }));
