...
import { expectConfirmModalAndAct, expectModalWithTitle, expectToClickDetailsPageOption, expectToClickModalAction, goToAdminConsole, waitForToast, } from '#src/ui-helpers/index.js';
import { expectNavigation, appendPathname, generateUsername, generateResourceName, generateResourceIndicator, generateScopeName, generateRoleName, } from '#src/utils.js';
...
describe('RBAC', () => {
    ...
    it('assign a role to a user on the role details page', async () => {
        ...
        await expect(page).toMatchElement('div[class$=usersTable] td div[class$=item] a[class$=title]', {
            text: rbacTestUsername,
        });
    });
