...
import { goToAdminConsole, expectToSaveChanges, waitForToast, expectToClickModalAction, expectToClickDetailsPageOption, expectModalWithTitle, expectConfirmModalAndAct, expectMainPageWithTitle, } from '#src/ui-helpers/index.js';
...
describe('webhooks', () => {
    ...
    it('can create a new webhook', async () => {
        ...
        const hookId = await page.$eval('div[class$=main] div[class$=metadata] div[class$=row] div:first-of-type', (element) => element.textContent);
        if (hookId) {
    ...
    it('can disable or enable a webhook', async () => {
        ...
        await expect(page).toMatchElement('div[class$=header] div[class$=metadata] div:nth-of-type(2) div[class$=outlined] div:nth-of-type(2)', {
            text: 'Not in use',
            timeout: 1000,
        });
        // Reactivate webhook
        ...
        await page.waitForSelector('div[class$=header] div[class$=metadata] div:nth-of-type(2) div[class$=state]');
    });
