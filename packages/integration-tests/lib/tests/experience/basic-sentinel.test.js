...
describe('basic sentinel', () => {
    ...
    it('should block a non-existing identifier after 5 failed attempts in 1 hour', async () => {
        ...
        await experience.toFillPasswordsToInputs({ inputNames: ['password'], shouldNavigate: false }, ['1', 'account or password'], ['2', 'account or password'], ['3', 'account or password'], ['4', 'account or password'], '5');
        await experience.waitForToast('Too many attempts');
        await experience.page.reload({ waitUntil: 'networkidle0' });
        await experience.toFillPasswordsToInputs({ inputNames: ['password'], shouldNavigate: false }, '6');
        await experience.waitForToast('Too many attempts');
    });
    it('should block failed attempts from both password and verification code', async () => {
        ...
        await experience.toFillPasswordsToInputs({ inputNames: ['password'], shouldNavigate: false }, ['1', 'account or password'], ['2', 'account or password'], ['3', 'account or password']);
...
//# sourceMappingURL=basic-sentinel.test.js.map