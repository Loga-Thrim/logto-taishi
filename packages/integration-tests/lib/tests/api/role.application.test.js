...
import { assignApplicationsToRole, createRole, deleteApplicationFromRole, getRoles, getRoleApplications, } from '#src/api/role.js';
describe('roles applications', () => {
    ...
    it('should return 404 if role not found', async () => {
        const response = await getRoleApplications('not-found').catch((error) => error);
    ...
    it('should fail when try to assign empty applications', async () => {
        const role = await createRole({ type: RoleType.MachineToMachine });
        const response = await assignApplicationsToRole([], role.id).catch((error) => error);
    ...
    it('should fail if role not found', async () => {
        const m2mApp = await createApplication(generateStandardId(), ApplicationType.MachineToMachine);
        const response = await assignApplicationsToRole([m2mApp.id], 'not-found').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should fail if application not found', async () => {
        const role = await createRole({ type: RoleType.MachineToMachine });
        const response = await assignApplicationsToRole(['not-found'], role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    ...
    it('should fail if role not found when trying to remove application from role', async () => {
        const m2mApp = await createApplication(generateStandardId(), ApplicationType.MachineToMachine);
        const response = await deleteApplicationFromRole(m2mApp.id, 'not-found').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should fail if application not found when trying to remove application from role', async () => {
        const role = await createRole({ type: RoleType.MachineToMachine });
        const response = await deleteApplicationFromRole('not-found', role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
});
//# sourceMappingURL=role.application.test.js.map