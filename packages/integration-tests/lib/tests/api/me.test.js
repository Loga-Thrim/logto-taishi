...
import { createUserWithAllRolesAndSignInToClient, deleteUser, resourceDefault, resourceMe, } from '#src/helpers/admin-tenant.js';
...
describe('me', () => {
    ...
    it('should only recognize the access token with correct resource and scope', async () => {
        ...
        await expectRejects(got.get(logtoConsoleUrl + '/me/custom-data', {
            headers: { authorization: `Bearer ${await client.getAccessToken(resourceDefault)}` },
        }), {
            code: 'auth.unauthorized',
            statusCode: 401,
        });
        await expect(got.get(logtoConsoleUrl + '/me/custom-data', {
