...
import { mockEmailConnectorConfig, mockEmailConnectorId, mockSmsConnectorConfig, mockSmsConnectorId, mockSocialConnectorConfig, mockSocialConnectorNewConfig, mockSocialConnectorId, mockAlternativeEmailConnectorConfig, mockAlternativeEmailConnectorId, } from '#src/__mocks__/connectors-mock.js';
import { deleteConnectorById, getConnector, listConnectors, postConnector, sendEmailTestMessage, sendSmsTestMessage, updateConnectorConfig, listConnectorFactories, getConnectorFactory, } from '#src/api/connector.js';
...
test('connector set-up flow', async () => {
    ...
    await Promise.all(mockConnectorSetups.map(async ({ connectorId }) => {
     ...
     */
    await Promise.all(mockConnectorSetups.map(async ({ connectorId, config }) => {
     ...
     */
    await expect(updateConnectorConfig(connectorIdMap.get(mockSocialConnectorId), mockSmsConnectorConfig)).rejects.toThrow(HTTPError);
    ...
    const { config: updatedConfig } = await updateConnectorConfig(connectorIdMap.get(mockSocialConnectorId), mockSocialConnectorNewConfig);
    ...
    expect(currentConnectors.some((connector) => connector.connectorId === mockEmailConnectorId)).toBeFalsy();
    connectorIdMap.delete(mockEmailConnectorId);
    expect(currentConnectors.find((connector) => connector.connectorId === mockAlternativeEmailConnectorId)
        ?.config).toEqual(mockAlternativeEmailConnectorConfig);
     ...
     */
    await expect(deleteConnectorById(connectorIdMap.get(mockAlternativeEmailConnectorId))).resolves.not.toThrow();
    await expect(getConnector(connectorIdMap.get(mockAlternativeEmailConnectorId))).rejects.toThrow(HTTPError);
    ...
    expect(allConnectorsAfterSetup.find(({ id }) => id === connectorIdMap.get(mockSmsConnectorId))).toEqual(expect.objectContaining({
        id: connectorIdMap.get(mockSmsConnectorId),
        connectorId: mockSmsConnectorId,
        config: mockSmsConnectorConfig,
    }));
    expect(allConnectorsAfterSetup.find(({ id }) => id === connectorIdMap.get(mockSocialConnectorId))).toEqual(expect.objectContaining({
...
test('create non standard social connector with target', async () => {
    await cleanUpConnectorTable();
    await expectRejects(postConnector({ connectorId: mockSocialConnectorId, metadata: { target: 'target' } }), {
...
test('send SMS/email test message', async () => {
    ...
    await expect(sendSmsTestMessage(mockSmsConnectorId, generatePhone, mockSmsConnectorConfig)).resolves.not.toThrow();
    await expect(sendEmailTestMessage(mockEmailConnectorId, generateEmail, mockEmailConnectorConfig)).resolves.not.toThrow();
    // Set up passwordless connectors.
    await Promise.all([{ connectorId: mockSmsConnectorId }, { connectorId: mockEmailConnectorId }].map(async ({ connectorId }) => {
    ...
    await expect(sendSmsTestMessage(mockSmsConnectorId, generatePhone, {})).rejects.toThrow(HTTPError);
    await expect(sendEmailTestMessage(mockEmailConnectorId, generateEmail, {})).rejects.toThrow(HTTPError);
