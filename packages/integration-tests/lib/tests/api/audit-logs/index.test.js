...
describe('audit logs for interaction', () => {
    ...
    it('should insert log after interaction started and ended', async () => {
        ...
        const createLogs = await getAuditLogs(new URLSearchParams({ logKey: `${interaction.prefix}.${interaction.Action.Create}` }));
        ...
        const endLogs = await getAuditLogs(new URLSearchParams({ logKey: `${interaction.prefix}.${interaction.Action.End}` }));
