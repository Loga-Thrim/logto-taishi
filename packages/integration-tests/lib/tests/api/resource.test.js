...
import { createResource, getResource, getResources, updateResource, deleteResource, setDefaultResource, } from '#src/api/index.js';
...
describe('admin console api resources', () => {
    ...
    it('should return 404 if resource not found', async () => {
        const response = await getResource('not_found').catch((error) => error);
    ...
    it('should delete api resource successfully', async () => {
        ...
        const response = await getResource(createdResource.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode === 404).toBe(true);
    });
    it('should throw 404 when delete api resource not found', async () => {
        const response = await deleteResource('dummy_id').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode === 404).toBe(true);
    });
