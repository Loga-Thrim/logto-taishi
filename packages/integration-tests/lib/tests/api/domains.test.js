...
describe('domains', () => {
    ...
    it('should fail when already has a domain', async () => {
        await createDomain();
        const response = await createDomain().catch((error) => error);
    ...
    it('should return 404 if domain does not exist', async () => {
        const response = await getDomain('non_existent_domain').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should delete domain successfully', async () => {
        const domain = await createDomain();
        await deleteDomain(domain.id);
        const response = await getDomain(domain.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
});
