...
import { assignScopesToRole, createRole, deleteScopeFromRole, getRoleScopes, } from '#src/api/role.js';
...
describe('roles scopes', () => {
    ...
    it('should return 404 if role not found', async () => {
        const response = await getRoleScopes('not-found').catch((error) => error);
    ...
    it('should fail when try to assign empty scopes', async () => {
        const role = await createRole({});
        const response = await assignScopesToRole([], role.id).catch((error) => error);
    ...
    it('should fail if role not found', async () => {
        const resource = await createResource();
        const scope = await createScope(resource.id);
        const response = await assignScopesToRole([scope.id], 'not-found').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should fail if scope not found', async () => {
        const role = await createRole({});
        const response = await assignScopesToRole(['not-found'], role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should fail if scope already assigned to role', async () => {
        ...
        const response = await assignScopesToRole([scope1.id, scope2.id], role.id).catch((error) => error);
    ...
    it('should fail when try to remove scope from role that is not assigned', async () => {
        ...
        const resource = await createResource();
        const scope = await createScope(resource.id);
        const response = await deleteScopeFromRole(scope.id, role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should fail when try to remove scope from role that is not found', async () => {
        const resource = await createResource();
        const scope = await createScope(resource.id);
        const response = await deleteScopeFromRole(scope.id, 'not-found').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should fail when try to assign a scope to an internal role', async () => {
        const resource = await createResource();
        const scope = await createScope(resource.id);
        const response = await assignScopesToRole([scope.id], defaultManagementApi.role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(403);
    });
});
//# sourceMappingURL=role.scope.test.js.map