...
import { createRole, deleteRole, getRole, getRoles, getRoleScopes, updateRole, } from '#src/api/role.js';
...
describe('roles', () => {
    ...
    it('should fail when create role with conflict name', async () => {
        ...
        const response = await createRole({ name }).catch((error) => error);
    ...
    it('should fail when try to create an internal role', async () => {
        const response = await createRole({ name: '#internal:foo' }).catch((error) => error);
    ...
    it('should return 404 if role does not exist', async () => {
        const response = await getRole('non_existent_role').catch((error) => error);
    ...
    it('should delete role successfully', async () => {
        ...
        const response = await getRole(role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
    it('should return 404 if role does not exist', async () => {
        const response = await deleteRole('non_existent_role').catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode).toBe(404);
    });
});
