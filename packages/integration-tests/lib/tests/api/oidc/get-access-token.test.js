...
describe('get access token', () => {
    ...
    it('can sign in and getAccessToken with admin user', async () => {
        ...
        expect(getAccessTokenPayload(accessToken)).toHaveProperty('scope', defaultManagementApi.scopes.map(({ name }) => name).join(' '));
    ...
    it('can sign in and getAccessToken with guest user', async () => {
        ...
        await processSession(client, redirectTo);
        const accessToken = await client.getAccessToken(defaultManagementApi.resource.indicator);
        expect(getAccessTokenPayload(accessToken)).not.toHaveProperty('scope', defaultManagementApi.scopes.map(({ name }) => name).join(' '));
    });
    it('can sign in and get multiple Access Tokens by the same Refresh Token within refreshTokenReuseInterval', async () => {
        ...
        const getAccessTokenByRefreshToken = async () => fetchTokenByRefreshToken({
...
//# sourceMappingURL=get-access-token.test.js.map