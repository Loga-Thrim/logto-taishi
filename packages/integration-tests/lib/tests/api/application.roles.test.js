...
import { createApplication, getApplicationRoles, assignRolesToApplication, deleteRoleFromApplication, putRolesToApplication, } from '#src/api/index.js';
...
describe('admin console application management (roles)', () => {
    ...
    it('throws when trying to get roles of non-m2m app', async () => {
        ...
        const application = await createApplication(generateStandardId(), applicationType);
        const response = await getApplicationRoles(application.id).catch((error) => error);
    ...
    it('should failed to delete non-existing-role from app', async () => {
        ...
        const application = await createApplication(generateStandardId(), applicationType);
        const role = await createRole({ type: RoleType.MachineToMachine });
        const response = await deleteRoleFromApplication(application.id, role.id).catch((error) => error);
        expect(response instanceof HTTPError && response.response.statusCode === 404).toBe(true);
    });
});
//# sourceMappingURL=application.roles.test.js.map