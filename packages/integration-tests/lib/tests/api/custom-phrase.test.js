...
import { listCustomPhrases, getCustomPhrase, createOrUpdateCustomPhrase, deleteCustomPhrase, } from '#src/api/custom-phrase.js';
...
describe('custom-phrase flow', () => {
    ...
    it('fail to update an existing custom phrase with invalid translation keys', async () => {
        await expect(createOrUpdateCustomPhrase('zh', { ...mockZhTranslationUpdated, invalidKey: 'invalid value' })).rejects.toThrow(HTTPError);
    });
    ...
    it('get all custom phrases', async () => {
        ...
        expect(allCustomPhrases.find(({ languageTag }) => languageTag === 'zh')?.translation).toEqual(mockZhTranslationUpdated);
        expect(allCustomPhrases.find(({ languageTag }) => languageTag === 'en-US')?.translation).toEqual(mockEnUsTranslation);
    });
...
//# sourceMappingURL=custom-phrase.test.js.map