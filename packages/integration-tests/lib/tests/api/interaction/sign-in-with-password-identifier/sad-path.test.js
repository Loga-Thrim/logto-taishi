...
describe('Sign-in flow sad path using password identifiers', () => {
    ...
    it('Should fail to sign-in with password if sign-in mode is register only', async () => {
        ...
        // Username & password
        const { userProfile: { username, password: password1 }, } = await generateNewUser({ username: true, password: true });
        ...
        // Email & password
        const { userProfile: { primaryEmail, password: password2 }, } = await generateNewUser({ primaryEmail: true, password: true });
        await expectRejects(client.send(putInteraction, {
            event: InteractionEvent.SignIn,
        ...
        // Phone & password
        const { userProfile: { primaryPhone, password: password3 }, } = await generateNewUser({ primaryPhone: true, password: true });
        await expectRejects(client.send(putInteraction, {
            event: InteractionEvent.SignIn,
    ...
    it('Should fail to sign-in with username and password if user password is not correct', async () => {
        const { userProfile: { username }, } = await generateNewUser({ username: true, password: true });
        const client = await initClient();
        await expectRejects(client.send(putInteraction, {
            event: InteractionEvent.SignIn,
    ...
    it('Should fail to sign-in with username and password if user password is not set', async () => {
        const { userProfile: { username }, } = await generateNewUser({ username: true, primaryEmail: true });
        const client = await initClient();
        await expectRejects(client.send(putInteraction, {
            event: InteractionEvent.SignIn,
            identifier: { username, password: generatePassword() },
    ...
    it('Should fail to sign-in with username and password if the user is suspended', async () => {
        const { user, userProfile: { username, password }, } = await generateNewUser({ username: true, password: true });
