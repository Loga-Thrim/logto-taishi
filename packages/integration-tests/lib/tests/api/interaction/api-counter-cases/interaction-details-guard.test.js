...
import { putInteractionEvent, patchInteractionIdentifiers, putInteractionProfile, patchInteractionProfile, deleteInteractionProfile, createSocialAuthorizationUri, sendVerificationCode, putInteraction, deleteInteraction, } from '#src/api/interaction.js';
...
describe('Interaction details guard checking', () => {
    ...
    it('PUT /interaction/event', async () => {
        await expectRejects(client.send(putInteractionEvent, {
            event: InteractionEvent.SignIn,
        }), {
            code: 'session.not_found',
            statusCode: 400,
    ...
    it('PUT /interaction/profile', async () => {
        await expectRejects(client.send(putInteractionProfile, {
            username: generateUsername(),
            password: generatePassword(),
        }), {
            code: 'session.not_found',
    ...
    it('PATCH /interaction/profile', async () => {
        await expectRejects(client.send(patchInteractionProfile, {
            username: generateUsername(),
            password: generatePassword(),
        }), {
            code: 'session.not_found',
    ...
    it('POST /interaction/verification/social-authorization-uri', async () => {
        await expectRejects(client.send(createSocialAuthorizationUri, {
            state: 'fake_state',
...
//# sourceMappingURL=interaction-details-guard.test.js.map