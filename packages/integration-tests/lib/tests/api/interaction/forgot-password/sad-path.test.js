...
import { patchInteractionIdentifiers, putInteraction, putInteractionProfile, sendVerificationCode, } from '#src/api/interaction.js';
...
describe('reset password flow sad path', () => {
    ...
    it('Should fail to reset password if related user is suspended', async () => {
        await setSmsConnector();
        const { user, userProfile: { primaryPhone }, } = await generateNewUser({
            primaryPhone: true,
            password: true,
        });
