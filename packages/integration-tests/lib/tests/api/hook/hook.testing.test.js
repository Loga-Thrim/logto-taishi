...
describe('hook testing', () => {
    ...
    const { listen: listenError, close: closeError } = createMockServer(responseErrorPort, (request, response) => {
    ...
    it('should return 404 if the hook to test does not exist', async () => {
        ...
        await expectRejects(authedAdminApi.post(`hooks/${invalidHookId}/test`, {
            json: { events: [HookEvent.PostSignIn], config: { url: responseSuccessEndpoint } },
        }), {
    ...
    it('should return 422 if the hook endpoint is not working', async () => {
        ...
        const created = await authedAdminApi.post('hooks', { json: payload }).json();
        await expectRejects(authedAdminApi.post(`hooks/${created.id}/test`, {
...
//# sourceMappingURL=hook.testing.test.js.map