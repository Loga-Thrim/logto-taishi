...
export default class ExpectExperience extends ExpectPage {
     ...
     */
    async startWith(initialUrl = demoAppUrl, type = 'sign-in') {
     ...
     */
    async toCompleteVerification(type) {
     ...
     */
    async toFillVerificationCode(code) {
     ...
     */
    async toFillNewPasswords(...passwords) {
     ...
     * ```
     */
    async toFillPasswordsToInputs({ inputNames, shouldNavigate = true }, ...passwords) {
        for (const element of passwords) {
            ...
            if (errorMessage === undefined) {
                ...
                // eslint-disable-next-line no-await-in-loop
                await this.toMatchAlert(typeof errorMessage === 'string' ? new RegExp(errorMessage, 'i') : errorMessage);
            }
        }
    }
    /**
