"use strict";
var __makeTemplateObject = (this && this.__makeTemplateObject) || function (cooked, raw) {
    if (Object.defineProperty) { Object.defineProperty(cooked, "raw", { value: raw }); } else { cooked.raw = raw; }
    return cooked;
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var slonik_1 = require("slonik");
var alteration = {
    up: function (pool) { return __awaiter(void 0, void 0, void 0, function () {
        var applications, roles, _i, applications_1, application, _loop_1, _a, _b, roleName;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0: return [4 /*yield*/, pool.query((0, slonik_1.sql)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["\n      create table applications_roles (\n        application_id varchar(21) not null references applications (id) on update cascade on delete cascade,\n        role_id varchar(21) not null references roles (id) on update cascade on delete cascade,\n        primary key (application_id, role_id)\n      );\n    "], ["\n      create table applications_roles (\n        application_id varchar(21) not null references applications (id) on update cascade on delete cascade,\n        role_id varchar(21) not null references roles (id) on update cascade on delete cascade,\n        primary key (application_id, role_id)\n      );\n    "]))))];
                case 1:
                    _c.sent();
                    return [4 /*yield*/, pool.any((0, slonik_1.sql)(templateObject_2 || (templateObject_2 = __makeTemplateObject(["\n      select * from applications where jsonb_array_length(role_names) > 0\n    "], ["\n      select * from applications where jsonb_array_length(role_names) > 0\n    "]))))];
                case 2:
                    applications = _c.sent();
                    return [4 /*yield*/, pool.any((0, slonik_1.sql)(templateObject_3 || (templateObject_3 = __makeTemplateObject(["\n      select * from roles\n    "], ["\n      select * from roles\n    "]))))];
                case 3:
                    roles = _c.sent();
                    _i = 0, applications_1 = applications;
                    _c.label = 4;
                case 4:
                    if (!(_i < applications_1.length)) return [3 /*break*/, 9];
                    application = applications_1[_i];
                    _loop_1 = function (roleName) {
                        var role;
                        return __generator(this, function (_d) {
                            switch (_d.label) {
                                case 0:
                                    if (!roleName) {
                                        return [2 /*return*/, "continue"];
                                    }
                                    role = roles.find(function (_a) {
                                        var name = _a.name;
                                        return name === roleName;
                                    });
                                    if (!role) {
                                        throw new Error("Unable to find role: ".concat(roleName));
                                    }
                                    // eslint-disable-next-line no-await-in-loop
                                    return [4 /*yield*/, pool.query((0, slonik_1.sql)(templateObject_4 || (templateObject_4 = __makeTemplateObject(["\n          insert into applications_roles (application_id, role_id) values (", ", ", ")\n        "], ["\n          insert into applications_roles (application_id, role_id) values (", ", ", ")\n        "])), application.id, role.id))];
                                case 1:
                                    // eslint-disable-next-line no-await-in-loop
                                    _d.sent();
                                    return [2 /*return*/];
                            }
                        });
                    };
                    _a = 0, _b = application.roleNames;
                    _c.label = 5;
                case 5:
                    if (!(_a < _b.length)) return [3 /*break*/, 8];
                    roleName = _b[_a];
                    return [5 /*yield**/, _loop_1(roleName)];
                case 6:
                    _c.sent();
                    _c.label = 7;
                case 7:
                    _a++;
                    return [3 /*break*/, 5];
                case 8:
                    _i++;
                    return [3 /*break*/, 4];
                case 9: return [4 /*yield*/, pool.query((0, slonik_1.sql)(templateObject_5 || (templateObject_5 = __makeTemplateObject(["\n      alter table applications drop column role_names\n    "], ["\n      alter table applications drop column role_names\n    "]))))];
                case 10:
                    _c.sent();
                    return [2 /*return*/];
            }
        });
    }); },
    down: function (pool) { return __awaiter(void 0, void 0, void 0, function () {
        var relations, roles, _loop_2, _i, relations_1, relation;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, pool.query((0, slonik_1.sql)(templateObject_6 || (templateObject_6 = __makeTemplateObject(["\n      alter table applications add column role_names jsonb not null default '[]'::jsonb\n    "], ["\n      alter table applications add column role_names jsonb not null default '[]'::jsonb\n    "]))))];
                case 1:
                    _a.sent();
                    return [4 /*yield*/, pool.any((0, slonik_1.sql)(templateObject_7 || (templateObject_7 = __makeTemplateObject(["\n      select * from applications_roles\n    "], ["\n      select * from applications_roles\n    "]))))];
                case 2:
                    relations = _a.sent();
                    return [4 /*yield*/, pool.any((0, slonik_1.sql)(templateObject_8 || (templateObject_8 = __makeTemplateObject(["\n      select * from roles\n    "], ["\n      select * from roles\n    "]))))];
                case 3:
                    roles = _a.sent();
                    _loop_2 = function (relation) {
                        var role;
                        return __generator(this, function (_b) {
                            switch (_b.label) {
                                case 0:
                                    role = roles.find(function (_a) {
                                        var id = _a.id;
                                        return id === relation.roleId;
                                    });
                                    if (!role) {
                                        return [2 /*return*/, "continue"];
                                    }
                                    // eslint-disable-next-line no-await-in-loop
                                    return [4 /*yield*/, pool.query((0, slonik_1.sql)(templateObject_9 || (templateObject_9 = __makeTemplateObject(["\n        update applications set role_names = role_names || '[", "]'::jsonb where id = ", "\n      "], ["\n        update applications set role_names = role_names || '[", "]'::jsonb where id = ", "\n      "])), role.name, relation.applicationId))];
                                case 1:
                                    // eslint-disable-next-line no-await-in-loop
                                    _b.sent();
                                    return [2 /*return*/];
                            }
                        });
                    };
                    _i = 0, relations_1 = relations;
                    _a.label = 4;
                case 4:
                    if (!(_i < relations_1.length)) return [3 /*break*/, 7];
                    relation = relations_1[_i];
                    return [5 /*yield**/, _loop_2(relation)];
                case 5:
                    _a.sent();
                    _a.label = 6;
                case 6:
                    _i++;
                    return [3 /*break*/, 4];
                case 7: return [4 /*yield*/, pool.query((0, slonik_1.sql)(templateObject_10 || (templateObject_10 = __makeTemplateObject(["\n      drop table applications_roles;\n    "], ["\n      drop table applications_roles;\n    "]))))];
                case 8:
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    }); },
};
exports.default = alteration;
var templateObject_1, templateObject_2, templateObject_3, templateObject_4, templateObject_5, templateObject_6, templateObject_7, templateObject_8, templateObject_9, templateObject_10;
